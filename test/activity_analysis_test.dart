...
void main() {
  ...
  ActivityAnalysis run({
    ...
    DateTime? clock,
      ...
      ActivityAnalysis.compute(
        ...
        now: clock ?? at(15), // mid-afternoon by default
        dailyGoal: dailyGoal,
      );
  ...
  group('weekly/daily percentage is never clamped', () {
    ...
    test('weekly percentage can exceed 100 too', () {
      // 7 prior days at 12k each + 12k today → 84k vs 70k weekly goal = 120%.
      final all = <ActivitySample>[];
      for (var d = 1; d <= 6; d++) {
        all.add(_s(at(10).subtract(Duration(days: d)), steps: 12000));
      }
  ...
  group('active minutes are intensity/step based, never a named sport', () {
    test('a single stray step does not make an active minute', () {
      final today = [
        _s(at(9, 0), steps: 1), // stray — below cadence floor, low intensity
        _s(at(9, 1), steps: 30), // brisk cadence → active
        _s(at(9, 2), intensity: 40), // movement intensity → active
        _s(at(9, 3), intensity: 90), // high-intensity (also active)
      ];
      final a = run(liveSteps: 31, today: today);
      expect(a.activeMinutes, 3);
      expect(a.highIntensityMinutes, 1);
      // No insight/recommendation invents a sport name.
      final text =
          (a.insights.map((i) => i.text).toList() + a.recommendations)
              .join(' ')
              .toLowerCase();
    ...
    test('sleep minutes are excluded from active counts', () {
      ...
      final a = run(liveSteps: 100, today: today);
      expect(a.activeMinutes, 1);
    });
  });

  group('longest inactive stretch — real, gap-aware, sleep-excluded', () {
    test('counts a contiguous waking sit and ignores data gaps', () {
      final today = [
        // 09:00–09:04: five still minutes (steps 0, low intensity) = 5 min sit
        for (var m = 0; m < 5; m++) _s(at(9, m), steps: 0, intensity: 1),
        _s(at(9, 5), steps: 40), // movement breaks the run
        // big data gap, then two still minutes (only 2 min, not joined to above)
        _s(at(13, 0), steps: 0, intensity: 0),
        _s(at(13, 1), steps: 0, intensity: 0),
      ];
      final a = run(liveSteps: 40, today: today);
    ...
    test('a night of sleep does not count as sitting', () {
      final today = [
        for (var m = 0; m < 120; m++)
          _s(at(2, 0).add(Duration(minutes: m)),
              steps: 0, intensity: 0, category: 112), // all sleep
        _s(at(9, 0), steps: 30),
      ];
      final a = run(liveSteps: 30, today: today);
      expect(a.longestInactiveMin, 0);
    });
  });

  group('peak / least-active hour', () {
    test('peak hour from hourly steps; least-active ignores sleeping hours', () {
      final today = [
        _s(at(8, 0), steps: 10),
        _s(at(17, 0), steps: 10),
      ];
  ...
  group('baseline gate — comparisons/streaks suppressed until enough days', () {
    ...
    test('streak breaks on a missing (un-synced) day, never skips it', () {
      // Anchor a synthetic "today" well after the cutoff so the streak days are
      // all post-cutoff regardless of the real wall-clock date.
      final base = DateTime(
          Baseline.cutoff.year, Baseline.cutoff.month, Baseline.cutoff.day);
      final synthToday = base.add(const Duration(days: 20));
      final all = <ActivitySample>[];
      // Seed 8 distinct early post-cutoff days (cutoff+0..+7) to pass the gate.
      for (var i = 0; i < 8; i++) {
        all.add(_s(base.add(Duration(days: i, hours: 12)), steps: 3000));
      }
      // Recent completed days: y-1=goal, y-2=goal, y-3 MISSING, y-4=goal.
  ...
  group('no floors metric exists (no altimeter on MB6)', () {
    test('score components never include a floors/elevation component', () {
      ...
      final a = run(liveSteps: 6000, today: today);
      final labels =
  ...
  group('activity score decomposes into shown, weighted components', () {
    test('score equals the weighted sum of its components', () {
      ...
      final a = run(liveSteps: 6000, today: today, dailyGoal: 10000);
