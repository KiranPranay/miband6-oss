...
| `findings-23.md` | **The stress data was never stress** — all 26 863 stored readings decode as 8-byte activity records (a "score of 64" is that minute's HR byte); ingest quarantined behind a probe, history rebuilt on the heart-rate estimate. Also: the night reported as 8h41m/45% was 6h29m/61%, stitched together by three samples the kind-byte fallback should never have accepted. |
| `findings-26.md` | **A night the band did not flag is still a night.** Detection anchored on the band's sleep flag alone; a night worn end-to-end at 97% HR coverage with the flag on 20% of minutes produced *nothing*. Chinoy actigraphy now anchors too, corroborated per minute by heart rate below the waking median, with sustained onset and a 12-hour split. 53 → 56 nights, the missing one recovered, no session over 12 h, efficiency unchanged. |
| `findings-25.md` | **Deep sleep returns, as an estimate with a model behind it.** A whole-session linear detrend removes the circadian drift without the edge bias that hid cycle-1 troughs, and Borbély's Process S makes the required dip grow across the night — front-loading by construction. On 53 real nights: 14.2% median share, bout position 0.446, one empty night. Every surface says "est."; score weight 20%, below both measured components. |
| `findings-24.md` | **The deep-sleep detector does not find deep sleep** — its output is uniform across the night (219/205/211 across night-thirds) because it thresholds against a rolling median centred on its own data, while slow-wave sleep is front-loaded. No parameter choice moves the position; the share could have been tuned into the 13-23% band without becoming true. Staging quarantined, the stage withdrawn, and the 30% it carried in the sleep score removed. |
